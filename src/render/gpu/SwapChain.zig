...
fn_present: ?*const fn (*Self) void = null,
...
fn_resizeBuffers: ?*const fn (*Self, [2]u32) bool = null,
...
fn _getNumColourAttachments(rt: *const RenderTarget) u32 {
    return fromBaseConst(rt).getNumColourAttachments();
}
...
fn _hasDepthAttachment(rt: *const RenderTarget) bool {
    return fromBaseConst(rt).hasDepthAttachment();
}
...
fn _hasStencilAttachment(rt: *const RenderTarget) bool {
    return fromBaseConst(rt).hasStencilAttachment();
}
...
pub fn resizeBuffers(self: *Self, resolution: [2]u32, resize_info: SwapChainResizeInfo) bool {
    if (resize_info.modify_surface) {
        if (self.surface) |s| {
            s.setFullscreenMode(if (resize_info.fullscreen) .fullscreen else .windowed);
            s.setSize(resolution);
            s.update();
            if (self.fn_resizeBuffers.?(self, resolution)) {
